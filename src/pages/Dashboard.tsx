...
export default function Dashboard() {
  ...
  // Tema com detecção automática do localStorage
  ...
  const deleteNote = async (id: string) => {
    if (confirm("Apagar nota permanentemente?")) {
      ...
      setIsNoteEditorOpen(false); fetchData();
    }
  };
    ...
    <div className={`pb-28 pt-4 px-4 max-w-md mx-auto min-h-screen font-sans transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="flex items-center justify-between mb-4 bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 transition-colors">
        ...
        <div className="flex flex-col items-center">
          <h2 className="font-black text-lg capitalize leading-none dark:text-white">{format(currentDate, 'MMMM yyyy', { locale: ptBR })}</h2>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab + currentDate.toISOString()} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
          ...
          {activeTab === 'list' && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-1 px-1">
                <button onClick={() => setFilterUserId(null)} className={`px-4 py-2 rounded-full text-[10px] font-black uppercase transition-all ${!filterUserId ? 'bg-slate-800 dark:bg-white text-white dark:text-slate-900 shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-400 border border-slate-100 dark:border-slate-700'}`}>Todos</button>
              ...
              <div className="space-y-3 pb-4">
                {isLoading ? [1,2,3].map(i => <SkeletonCard key={i}/>) : 
                  stats.filtered.map((exp) => (
                    <motion.div layout key={exp.id} className="bg-white dark:bg-slate-800 p-4 rounded-[2rem] border border-slate-50 dark:border-slate-700 shadow-sm flex items-center justify-between transition-colors">
                      <div className="flex items-center gap-3 flex-1 overflow-hidden">
                        <img src={exp.profiles?.avatar_url || `https://ui-avatars.com/api/?name=${exp.profiles?.full_name}`} className="w-12 h-12 rounded-2xl object-cover border-2 border-slate-50 dark:border-slate-700" alt="" />
                        <div className="flex flex-col min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                             ...
                             {/* ÍCONE DE COMPROVANTE BEM VISÍVEL */}
                             {exp.receipt_url && (
                               <button onClick={() => setViewingReceipt(exp.receipt_url)} className="flex items-center gap-1 text-blue-500 bg-blue-50 dark:bg-blue-900/40 px-2 py-0.5 rounded-lg border border-blue-100 dark:border-blue-900 shadow-sm transition-all active:scale-90">
                      ...
                      <div className="text-right ml-2 shrink-0">
                        ...
                        <div className="flex gap-3 justify-end">
                          <button onClick={() => { setEditingId(exp.id); setAmount(new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(exp.amount)); setCategory(exp.category_name); setDescription(exp.description); setReceiptUrl(exp.receipt_url || ''); setActiveTab('add'); }} className="text-slate-300 hover:text-blue-500 transition-colors"><Edit2 size={16}/></button>
                          <button onClick={async () => { if(confirm("Remover este gasto?")) { await supabase.from('expenses').update({ is_deleted: true }).eq('id', exp.id); fetchData(); } }} className="text-slate-300 hover:text-red-400 transition-colors"><Trash2 size={16}/></button>
                        </div>
                      </div>
                    </motion.div>
                ))}
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-700 space-y-6 transition-colors">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="relative">
                  ...
                  <input type="text" inputMode="numeric" placeholder="0,00" required className="w-full p-6 pl-16 bg-slate-50 dark:bg-slate-900/50 rounded-3xl text-4xl font-black outline-none text-blue-600 border-2 border-transparent focus:border-blue-500 transition-all shadow-inner dark:text-white" value={amount} onChange={handleCurrencyChange} />
                </div>
                ...
                <input type="text" placeholder="O que você comprou?" className="w-full p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700 outline-none text-slate-800 dark:text-slate-200" value={description} onChange={(e) => setDescription(e.target.value)} />
                <div className="flex items-center gap-4">
                  <label className="flex-1 flex items-center justify-center gap-2 p-4 bg-slate-100 dark:bg-slate-700 rounded-2xl text-[10px] font-black uppercase cursor-pointer dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
                  ...
                  {receiptUrl && <button type="button" onClick={() => setReceiptUrl('')} className="p-4 bg-red-50 dark:bg-red-900/30 text-red-500 rounded-2xl transition-all active:scale-90"><X size={16}/></button>}
                </div>
                <button className="w-full bg-blue-600 text-white font-black py-5 rounded-3xl shadow-xl shadow-blue-500/30 active:scale-95 transition-all uppercase tracking-widest">Salvar Registro</button>
              </form>
            </div>
          )}

          {/* ... (Telas de Shopping, Notes, Reminders e Stats adaptadas) ... */}
          {activeTab === 'shopping' && (
            <div className="space-y-6">
              <form onSubmit={async (e) => { e.preventDefault(); if(!newItem) return; await supabase.from('shopping_list').insert({ item_name: newItem, user_id: user.id }); setNewItem(''); fetchData(); }} className="flex gap-2">
                ...
                <button className="bg-blue-600 text-white p-4 rounded-2xl active:scale-90 transition-all"><Plus size={24} /></button>
              </form>
                  ...
                  <div key={item.id} className={`bg-white dark:bg-slate-800 p-4 rounded-2xl flex items-center justify-between border transition-all ${!item.is_pending ? 'opacity-50 bg-slate-50 dark:bg-slate-900/50' : 'border-slate-100 dark:border-slate-700 shadow-sm'}`}>
                    <div className="flex items-center gap-3 flex-1">
                      <button onClick={async () => { await supabase.from('shopping_list').update({ is_pending: !item.is_pending }).eq('id', item.id); fetchData(); }}>
                        {item.is_pending ? <Circle size={24} className="text-slate-300 dark:text-slate-600" /> : <CheckCircle2 size={24} className="text-emerald-500" />}
                      </button>
                      <div className="flex flex-col"><span className={`font-bold text-sm ${!item.is_pending ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}>{item.item_name}</span><span className="text-[9px] text-slate-400 font-black uppercase tracking-widest leading-none">De: {item.profiles?.full_name?.split(' ')[0]}</span></div>
                    </div>
                    <button onClick={async () => { if(confirm("Apagar?")) { await supabase.from('shopping_list').delete().eq('id', item.id); fetchData(); } }} className="text-slate-300 hover:text-red-500 p-2 transition-colors"><Trash2 size={16} /></button>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'notes' && (
            <div className="space-y-4">
              <button onClick={() => { setEditingNoteId(null); setNoteTitle(''); setNoteContent(''); setIsNoteEditorOpen(true); }} className="w-full p-5 bg-amber-400 text-slate-900 rounded-[2rem] font-black shadow-lg flex items-center justify-center gap-2 active:scale-95 uppercase text-xs tracking-widest transition-all"><Plus size={20} /> Criar Anotação</button>
              <div className="grid grid-cols-1 gap-3 pb-10">
                {notes.map((note) => (
                  <motion.button layout key={note.id} onClick={() => { setEditingNoteId(note.id); setNoteTitle(note.title); setNoteContent(note.content); setIsNoteEditorOpen(true); }} className="bg-white dark:bg-slate-800 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-700 shadow-sm text-left flex items-center justify-between transition-colors">
          ...
          {activeTab === 'reminders' && (
            <div className="space-y-6">
              <form onSubmit={handleReminderSubmit} className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-4 transition-colors">
                ...
                <input className="w-full p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl outline-none border border-slate-100 dark:border-slate-700 text-slate-800 dark:text-white" placeholder="Lembrar de?" required value={remText} onChange={e => setRemText(e.target.value)} />
                <div className="flex gap-2">
                  <div className="flex-1 relative"><Calendar size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/><input type="date" required className="w-full p-4 pl-12 bg-slate-50 dark:bg-slate-900/50 rounded-2xl outline-none border border-slate-100 dark:border-slate-700 text-sm dark:text-white" value={remDate} onChange={e => setRemDate(e.target.value)} /></div>
                  <div className="w-32 relative"><Clock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/><input type="time" className="w-full p-4 pl-12 bg-slate-50 dark:bg-slate-900/50 rounded-2xl outline-none border border-slate-100 dark:border-slate-700 text-sm dark:text-white" value={remTime} onChange={e => setRemTime(e.target.value)} /></div>
                </div>
                <button className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl shadow-lg active:scale-95 transition-all">Confirmar</button>
              </form>
              <div className="space-y-3 pb-10">
                {reminders.map((rem) => (
                  <div key={rem.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex items-center justify-between group transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="font-bold text-slate-800 dark:text-slate-200 text-sm leading-tight">{rem.text}</p><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">{format(parseISO(rem.reminder_date), "dd/MM", { locale: ptBR })} {rem.reminder_time && ` às ${rem.reminder_time}`}</p></div>
                    </div>
                    <div className="flex gap-2"><button onClick={() => {setEditRemId(rem.id); setRemText(rem.text); setRemDate(rem.reminder_date); setRemTime(rem.reminder_time || '');}} className="text-blue-500 transition-colors"><Edit2 size={16}/></button><button onClick={async () => { if(confirm("Apagar?")) { await supabase.from('reminders').delete().eq('id', rem.id); fetchData(); } }} className="text-red-400 transition-colors"><Trash2 size={16}/></button></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'stats' && (
            <div className="space-y-6 pb-10 px-1">
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
                ...
                <div className="mt-4 space-y-3">{userTotals.map((item: any, i: number) => (<div key={i} className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-700 transition-colors"><div className="flex items-center gap-3"><img src={item.avatar || `https://ui-avatars.com/api/?name=${item.name}`} className="w-8 h-8 rounded-full object-cover border-2 border-white dark:border-slate-700" /><span className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest leading-none">{item.name.split(' ')[0]}</span></div><span className="font-black text-slate-900 dark:text-white text-sm">{formatCurrency(item.value)}</span></div>))}</div>
              </div>
          ...
          {activeTab === 'config' && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex items-center gap-4 transition-colors">
                 ...
                 <div className="flex-1"><p className="font-bold text-slate-800 dark:text-slate-200 leading-tight">{userProfile?.full_name || 'Meu Perfil'}</p><button onClick={() => window.location.hash = '/profile'} className="text-blue-600 text-xs font-black uppercase tracking-widest mt-1">Editar Perfil →</button></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-6 bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 flex flex-col items-center gap-2 active:scale-95 transition-all text-slate-800 dark:text-slate-200 shadow-sm">
                   {isDarkMode ? <Sun className="text-amber-400" /> : <Moon className="text-indigo-500" />}
                   <span className="text-[10px] font-black uppercase tracking-widest leading-none">{isDarkMode ? 'MODO CLARO' : 'MODO ESCURO'}</span>
                </button>
                <button onClick={() => supabase.auth.signOut()} className="p-6 bg-red-50 dark:bg-red-900/20 rounded-[2rem] border border-transparent flex flex-col items-center gap-2 active:scale-95 transition-all text-red-500 shadow-sm">
                   <LogOut size={20} />
      ...
      <AnimatePresence>
        {isNoteEditorOpen && (
          <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} className="fixed inset-0 bg-white dark:bg-slate-900 z-[100] flex flex-col p-6 transition-colors">
            <div className="flex items-center justify-between mb-8"><button onClick={() => setIsNoteEditorOpen(false)} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500 transition-colors"><X size={24}/></button><h3 className="font-black text-slate-800 dark:text-slate-200 uppercase text-sm">Anotação</h3><button onClick={saveNote} className="p-3 bg-blue-600 rounded-full text-white shadow-lg transition-transform active:scale-90"><Save size={24}/></button></div>
            <input className="text-3xl font-black text-slate-800 dark:text-white outline-none mb-6 bg-transparent" placeholder="Título..." value={noteTitle} onChange={e => setNoteTitle(e.target.value)} />
            <textarea className="flex-1 w-full outline-none text-slate-600 dark:text-slate-400 text-lg resize-none bg-transparent" placeholder="Escreva..." value={noteContent} onChange={e => setNoteContent(e.target.value)} />
            {editingNoteId && <button onClick={() => deleteNote(editingNoteId)} className="mt-4 p-4 text-red-400 font-black text-[10px] border-2 border-red-50 dark:border-red-900/50 rounded-2xl uppercase tracking-widest active:scale-95 transition-all">Apagar Nota</button>}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {viewingReceipt && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/95 z-[200] flex flex-col p-4 backdrop-blur-xl">
            ...
            <p className="text-center text-white/50 text-[10px] font-black uppercase tracking-[0.4em] mt-6 leading-none">DOCUMENTO SALVO NO SUPABASE</p>
          </motion.div>
        )}
      </AnimatePresence>
...
function CategoryManager({ categories, refresh, formatCurrency }: any) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <input className="flex-1 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700 text-sm outline-none font-bold text-slate-800 dark:text-white" placeholder="Nova..." value={newCat} onChange={e => setNewCat(e.target.value)} />
        <button onClick={addCategory} className="bg-blue-600 text-white p-4 rounded-2xl active:scale-90 transition-all"><Plus size={20}/></button>
      </div>
        ...
        {categories.map((c: any) => (
          <div key={c.id} className="p-4 bg-slate-50 dark:bg-slate-900 rounded-[2rem] space-y-3 border border-slate-100 dark:border-slate-700 transition-colors">
            <div className="flex justify-between items-center">
              <input type="text" className="bg-transparent text-sm font-black outline-none uppercase tracking-tighter w-full text-slate-800 dark:text-slate-200 border-b border-transparent focus:border-blue-500" defaultValue={c.name} onBlur={(e) => handleNameChange(c.id, c.name, e.target.value)} />
              <button onClick={async () => { if(confirm("Apagar?")) { await supabase.from('categories').delete().eq('id', c.id); refresh(); } }} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={16}/></button>
            </div>
            <div className="flex flex-col gap-1 bg-white dark:bg-slate-800 p-3 rounded-2xl shadow-inner transition-colors border border-slate-100 dark:border-slate-700">
